...
for (i in 1:length(start_times)) {
  for (receiver in unique(bass_det$Receiver_Serial)) {
    block_subset <- bass_det %>%
...
bass_17916 <- bass_det %>% filter(Tag_number == 17916) %>% 
  group_by(Receiver_Serial, Lat, Long) %>% 
  summarize(detections = n())
bass_17918 <- bass_det %>% filter(Tag_number == 17918) %>% 
  group_by(Receiver_Serial, Lat, Long) %>% 
  summarize(detections = n())
bass_17962 <- bass_det %>% filter(Tag_number == 17962) %>% 
  group_by(Receiver_Serial, Lat, Long) %>% 
  summarize(detections = n())
bass_17966 <- bass_det %>% filter(Tag_number == 17966) %>% 
  group_by(Receiver_Serial, Lat, Long) %>% 
  summarize(detections = n())
